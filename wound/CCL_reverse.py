...
def image_to_2d_bool_array_reverse(image):
	im2 = image.convert('L')
	arr = np.asarray(im2)
	arr = arr != 255
...
if __name__ == "__main__":
    ...
    #a = cv2.cvtColor(output, cv2.COLOR_GRAY2BGR)
    ...
    #             a[i][j] = [255,0,0]
    #         elif((a[i][j] == [2,2,2]).all()):
    #             a[i][j] = [0,255,0]  
    #         elif((a[i][j] == [3,3,3]).all()):
    #             a[i][j] = [0,0,255]  
    #         elif((a[i][j] == [4,4,4]).all()):
    #             a[i][j] = [0,255,255]
    print(output[0][0])
    for i in range (224):
        for j in range(224):
    ...
    cv2.imwrite("test_reverse.png",output)
